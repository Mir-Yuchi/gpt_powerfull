...
async def main(message: types.Message, state: FSMContext):
    ...
    match params[0]:
        ...
        case "Фото":
            ...
            await message.answer("Отправьте одно фото, которое будет прикрепленно к рассылке",
                                 reply_markup=kb.back_mrkp)
        ...
        case "Видео":
            if db.get_sender_data()['photo_id'] != None:
                ...
                await message.answer('Вы уже прикрепили фото к рассылке. Оно было удалено из рассылки, т.к. '
                                     'бот не может отправить видео и фото одним сообщением.')
            await message.answer("Отправьте одно видео, которое будет прикрепленно к рассылке",
                                 reply_markup=kb.back_mrkp)
